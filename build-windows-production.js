...
// 主打包流程
async function buildWindowsProduction() {
    try {
        console.log('📦 开始Windows生产环境打包...');
        ...
        if (fs.existsSync('dist')) {
            try {
                ...
                console.log('✅ 成功清理dist目录');
            } catch (error) {
                console.log('⚠️ 无法删除dist目录，尝试强制删除...');
                try {
                    // 使用Windows命令强制删除
                    const { execSync } = require('child_process');
                    execSync('cmd /c "rmdir /s /q dist"', { stdio: 'inherit' });
                    console.log('✅ 强制删除成功');
                } catch (forceError) {
                    console.log('❌ 强制删除失败，请手动删除dist目录后重试');
                    console.log('❌ 错误信息:', forceError.message);
                    throw new Error('无法清理dist目录，请手动删除后重试');
                }
            }
        }
        ...
        // 3. 构建 Electron 应用 (Windows)
        console.log('⚡ 构建 Windows Electron 应用...');
        ...
        console.log('✅ Windows生产环境打包完成！');
        console.log('📁 输出目录: dist/');
        console.log('🎉 Windows应用已准备就绪！');

    } catch (error) {
        console.error('❌ Windows打包失败:', error);
        process.exit(1);
    }
}
